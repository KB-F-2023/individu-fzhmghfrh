...
# Mencampur dua individu untuk menghasilkan keturunan
...
# Mengaplikasikan mutasi pada sebuah individu
...
# Menggunakan algoritma genetika untuk menyelesaikan TSP
def genetic_algorithm(distance_matrix, pop_size, num_generations, mutation_rate, k):
    ...
    for generation in range(num_generations):
        # Memilih orang tua dengan metode tournament selection
        parents = tournament_selection(population, k, distance_matrix)

        # Menghasilkan keturunan dengan mating
        offspring = []
        ...
        # Memilih orang tua terbaik untuk dijadikan elitism
        elite = min(population, key=lambda x: calculate_distance(x, distance_matrix))

        # Mengaplikasikan mutasi pada setiap keturunan
        for i in range(len(offspring)):
            if random.random() < mutation_rate:
                offspring[i] = mutate(offspring[i])

        # Menambahkan elitism ke populasi dan menghapus individu terburuk
